...
public class StructurePiece {
    ...
    public List<Tile> getJigsaws(JRand rand) {
        ...
        List<Tile> tiles = this.info.getTiles().stream().sorted(PieceInfo.TILE_SORTER)
                .filter(tile -> tile.getBlockState(paletteId).getBlock().getId() == Block.JIGSAW.getId())
        ...
        //Equivalent to Collections.shuffle(tiles, rand).
        for(int i = tiles.size(); i > 1; i--) {
            int a = i - 1, b = rand.nextInt(i);
            tiles.set(a, tiles.set(b, tiles.get(a)));
        }
